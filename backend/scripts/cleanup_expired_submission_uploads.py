"""清理过期或被替代的报告上传对象（契约 9.2 / 9.3 的孤立对象清理）。

对应 ``docs/api-contract.md`` 第 9 节：

- 领取「未完成、未清理」且（已超过确认窗口 或 已被替代）的会话，
  **并且**预签名 PUT 已过期并经过删除缓冲期
  （``SUBMISSION_UPLOAD_DELETE_BUFFER_SECONDS``，默认 1 小时）；
- 删除其孤立对象并**再次 HeadObject 复查**：确认对象不存在才标记 ``expired_at``；
  对象重新出现（晚到 PUT）或存储不可用时保持未过期，留给下一轮重试；
- **已完成会话（``completed_at`` 非空）及其报告对象绝不删除**；
...
用法（在 ``backend`` 目录下执行）::
    ...
    ..\\.venv\\Scripts\\python.exe scripts\\cleanup_expired_submission_uploads.py
...
from app.db import registry  # noqa: E402,F401 - 导入全部 ORM 模型
from app.db.session import get_session_factory  # noqa: E402
from app.modules.grading import service as grading_service  # noqa: E402
...
async def _run() -> int:
    ...
    try:
        ...
        async with factory() as session:
            try:
                cleaned = await grading_service.cleanup_expired_submission_uploads(
        ...
        print(f"[OK] 本次标记过期的报告上传会话：{cleaned} 条")
        if cleaned == 0:
            print("     没有需要清理的过期或被替代会话（或已在之前的运行中处理）。")
        return 0
    finally:
        storage.close()
