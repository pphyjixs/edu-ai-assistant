...
5. **有界循环。** 单个 Run 最多 4 轮模型调用、6 个工具调用、1 个写工具调用；超限后给出可解释失败，不能无限循环。
...
- prompt 中展示的目录总长度不超过 4 KiB，超限时公平截断 description，并在日志记录遗漏数量。
...
- 单个 Run 工具结果累计不超过 `32 KiB`，Skill 正文累计不超过 `16 KiB`。
