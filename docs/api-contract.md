...
- 存储对象：服务端在返回 `204` 前**尽力删除**资料对象；对象删除失败仅记日志、不影响响应（对象键不对外暴露，残留对象无 API 可达路径）。
- 删除与解析互斥：`PROCESSING` 中的资料同样可以删除；已下发解析的 Worker 若在删除后回写状态，按资料已被删除忽略其结果（见 5.5）。
...
1. **领取**：以原子方式领取一个 `PENDING` 的 `MATERIAL_PARSE` 任务（`UPDATE ... WHERE status='PENDING' ... RETURNING` 或等价的行锁方案），置为 `RUNNING`、记录 `started_at`、`progress` 从 0 开始。
...
5. **重试兼容**：Worker 领取前发现任务已被 5.3 重置属正常流转；Worker 回写结果前发现资料已被删除（5.2）则放弃回写，仅记日志。
6. **崩溃安全**：Worker 中断后任务停留于 `RUNNING`；5.3 的重试对“任务 `SUCCEEDED` 但资料未 `READY`”等异常窗口同样可重置（见 5.3 分流表），不要求 Worker 自身实现租约续期。
