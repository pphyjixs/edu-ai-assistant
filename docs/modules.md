...
解析 Worker（契约 5.5）：完成上传与重试解析后由后台任务执行解析，推进任务 `PENDING → RUNNING → SUCCEEDED/FAILED` 与资料 `PROCESSING → READY/FAILED`；解析结果（章节与知识点）一次性落库，失败只写安全摘要、不产生部分数据。第一版不做扫描版 PDF 的 OCR，也不提供通用 `POST /jobs/{job_id}/retry`。
